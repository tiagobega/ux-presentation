...
**A entrada não toca em nada dentro do SVG** — ela é uma varredura de `clip-path` na div que hospeda o desenho: um tween, um elemento, fora do SVG. A versão anterior animava a opacidade dos 118 elementos um a um, e `aplicarFase` anima a opacidade **dos mesmos elementos** com `overwrite: "auto"` — na montagem os dois rodavam juntos e cada um matava os tweens do outro no meio, o que lia como piscada e reinício. Duas animações escrevendo a mesma propriedade nos mesmos nós nunca vai ser estável. Por isso também a primeira `aplicarFase` é instantânea (`jaAplicou`): na montagem não há transição a fazer. O detector que fecha o caso conta **inversões de direção de opacidade por elemento** (subiu, desceu, subiu): 78 elementos antes, 0 depois.
