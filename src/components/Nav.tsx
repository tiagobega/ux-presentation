...
export default function Nav({
  ...
  return (
    <nav className="relative z-[100] bg-white/[0.94] backdrop-blur-md border-t border-black/[0.08] px-8 py-[13px] flex items-center justify-between gap-4 flex-shrink-0">
      <div
      ...
      <div className="flex items-center gap-2">
        <button className={btnBase} onClick={onPrev} disabled={isFirst} aria-label="Anterior">
          ←
        </button>
        <span className="font-mono text-[9px] text-text/[0.32] tracking-[0.08em] min-w-[80px] text-center whitespace-nowrap">
          {action} · {actionIndex + 1}/{totalActions}
        </span>
        <button className={btnBase} onClick={onNext} disabled={isLast} aria-label="Próximo">
          →
        </button>
        <button
