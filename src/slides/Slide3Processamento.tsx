...
function Card({
  ...
  if (!showResult)
    footer = { text: 'Aguardando análise', color: 'rgba(26,18,37,0.4)' };
  else if (rejected)
    footer = { text: img.bronzeReason ?? 'Reprovada', color: RED };
  else if (showOuro)
    footer =
      img.ouro === 'validada'
    ...
    footer = { text: img.detection ?? '', color: '#5b6b7c' };
  ...
  return (
    ...
    >
      <div
        className='relative flex-1 min-h-0  rounded-md border-2 transition-all duration-500'
        style={{ borderColor }}
      >
        {/* thumbnail */}
        {url ? (
          ...
          >
            <span className='text-[44px] font-bold text-text/15 font-mono'>
              {num}
            </span>
          </div>
        )}
        ...
        {showResult && (
          <motion.div
            ...
            className='absolute top-1.5 right-1.5 size-12 rounded-full flex items-center justify-center text-white'
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ duration: 0.3 }}
        className='flex items-center gap-2 font-medium leading-[1.3] px-0.5 truncate'
        style={{ color: footer.color }}
      >
        {phase === 0 && (
          <div>
            <Loader2 className='text-purple animate-spin size-3' />
          </div>
        )}
        {footer.text}
      </motion.div>
