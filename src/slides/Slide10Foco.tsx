  ...
  return (
    ...
    >
      <div>
        ...
        >
          Onde o <em className='not-italic text-purple'>UX entra</em> em cada
          frente.
        </motion.h1>
      </div>

      <div className='flex-1 min-h-0 flex flex-col gap-4 justify-center'>
        <div className='min-h-0 grid grid-cols-3 gap-4 items-center'>
          {fronts.map((f, i) => (
            <motion.div
              key={f.name}
            ...
            >
              ...
              <div className='p-8 flex flex-col gap-5 flex-1'>
                ...
                <div className='text-text/70 leading-[1.4]'>{f.sub}</div>
          ...
          <motion.div
            {...up(0.6, easeIn)}
            className='border border-purple/30 bg-purple/[0.07] px-9 py-5 col-span-3'
          >
      ...
      {/* thesis */}
      <motion.div {...up(0.8)}>
        <div className='text-[24px] font-bold text-text text-center tracking-[-0.02em] leading-[1.25]'>
          O objetivo não é apresentar mais uma ideia.{' '}
          <span className='text-purple'>
            É criar um eixo: experiência, comunicação e produto na mesma
            direção.
          </span>
        </div>
      </motion.div>
