...
export default function Slide02Fleets({ action: _ }: SlideProps) {
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 grid grid-cols-3 gap-16 items-center">
        {fronts.map((f, i) => (
          <motion.div
            key={f.name}
            {...up(0.3 + i * 0.1, easeIn)}
            className="border border-text/10 bg-black/2 p-12 flex flex-col gap-5"
          >
            ...
            <div className="text-[24px] text-text/80 leading-[1.45]">
              {f.desc}
            </div>
          </motion.div>
        ))}
